...
const Route = createFileRoute("/hostel-admin/in-out")({
  head: () => ({
    meta: [
      { title: "In-Out Register \u2014 Hostel Admin \xB7 CampusOS" },
      {
        name: "description",
        content: "Track hostel student entry & exit logs, scan QR at the gate, add manual entries and review late returns."
      }
    ]
  }),
  component: InOutPage
...
function InOutPage() {
  ...
  const filtered = useMemo(() => {
    return rows.filter((r) => {
      ...
      if (q) {
        ...
        if (!r.student.toLowerCase().includes(s) && !r.enrollment.toLowerCase().includes(s) && !r.room.toLowerCase().includes(s) && !r.destination.toLowerCase().includes(s))
          return false;
      }
      return true;
    });
  ...
  const filteredStudents = useMemo(() => {
    ...
    return students.filter(
      (st) => st.name.toLowerCase().includes(s) || st.enrollment.toLowerCase().includes(s) || st.room.toLowerCase().includes(s)
    ).slice(0, 12);
  }, [studentQ]);
  const registerExit = (student, form) => {
  ...
  />
    ...
    key={t.id}
    onClick={() => setTab(t.id)}
    className={`inline-flex items-center gap-1.5 rounded-lg px-3 py-1.5 text-sm font-medium transition-colors ${tab === t.id ? "text-white" : "text-muted-foreground hover:bg-muted hover:text-foreground"}`}
    style={tab === t.id ? { backgroundColor: TINT } : void 0}
  >
  ...
  >
    ...
    className={`rounded-full border px-3 py-1 text-xs font-medium transition-colors ${quick === f.id ? "border-transparent bg-[#7B4CED]/10 text-[#7B4CED]" : "border-border text-muted-foreground hover:bg-muted hover:text-foreground"}`}
  >
              {f.label}
            </button>)}
        </div>
  ...
  >
                  ...
                  <td className="whitespace-nowrap px-4 py-3 text-foreground">
                    {r.inTime ? <div className="flex flex-col leading-tight">
  ...
  >
                      ...
                      {(r.status === "Outside" || r.status === "Overdue") && <button
        ...
        <DialogContent className="max-w-2xl">
          {viewing && <>
              ...
              <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                ...
                <InfoRow icon={LogOut} label="Out" value={`${viewing.outDate} \xB7 ${viewing.outTime}`} />
                <InfoRow icon={Clock} label="Expected Return" value={viewing.expectedReturn} />
                <InfoRow
    icon={LogIn}
    label="Actual Return"
    value={viewing.inTime ? `${viewing.inDate} \xB7 ${viewing.inTime}` : "\u2014 Not returned"}
  />
              ...
              <div className="flex items-center justify-between rounded-lg border border-border p-3">
                ...
                {viewing.remarks && <p className="max-w-[60%] text-right text-xs text-muted-foreground">
                    {viewing.remarks}
                  </p>}
              </div>

              <DialogFooter>
                {(viewing.status === "Outside" || viewing.status === "Overdue") && <Button
    onClick={() => {
  ...
  />
    ...
    >
            ...
            {filteredStudents.length === 0 && <p className="p-6 text-center text-sm text-muted-foreground">No students found.</p>}
          </div>
        </DialogContent>
      </Dialog>
    </div>;
...
function ManualEntryDialog({
  ...
  const [gate, setGate] = useState("Hostel Gate");
  const submit = () => {
  ...
  >
    ...
    className="h-10 w-full rounded-lg border border-border bg-background px-3 text-sm text-foreground"
  >
              {["Class", "Personal", "Medical", "Home Visit", "Sports", "Library", "Other"].map(
