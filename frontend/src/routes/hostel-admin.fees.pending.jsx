...
const Route = createFileRoute("/hostel-admin/fees/pending")({
...
const FILTERS = ["All Unpaid", "Pending", "Overdue"];
function PendingFeesPage() {
  const [filter, setFilter] = useState("All Unpaid");
  ...
  />
    ...
    key={f}
    onClick={() => setFilter(f)}
    className={"rounded-full border px-3 py-1 text-xs font-medium transition-colors " + (filter === f ? "border-transparent bg-[#EF4444] text-white" : "border-border bg-card text-foreground hover:bg-muted")}
  >
                {f}
              </button>)}
          </div>
