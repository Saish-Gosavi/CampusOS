...
const CATEGORIES = ["All", "Bed", "Study Table", "Chair", "Wardrobe", "Fan", "Cupboard", "Mattress", "Desk Lamp", "Curtain", "Shelf"];
const STATUSES = ["All", "In Use", "In Storage", "Assigned", "Under Maintenance", "Replaced", "Retired"];
function FurnitureListPage() {
  ...
  const rows = useMemo(() => {
    ...
    if (query.trim()) {
      ...
      list = list.filter(
        (f) => f.code.toLowerCase().includes(q) || f.room.toLowerCase().includes(q) || f.hostel.toLowerCase().includes(q) || f.vendor.toLowerCase().includes(q) || (f.assignedTo ?? "").toLowerCase().includes(q)
      );
    }
    return list;
      ...
      <Dialog open={!!deleting} onOpenChange={(o) => !o && setDeleting(null)}>
        <DialogContent className="max-w-md">
          ...
          <DialogFooter>
            ...
            <Button className="bg-[#EF4444] text-white hover:bg-[#DC2626]" onClick={() => {
