...
const superAdminStats = [
  ...
  { label: "Total Admins", value: "179", delta: "+13 this month", trend: "up", icon: UserCog, tint: "#7B4CED" },
  { label: "System Roles", value: "9", delta: "Configured", trend: "up", icon: ShieldCheck, tint: "#3B82F6" },
  { label: "Total Students", value: "18,472", delta: "+312 this month", trend: "up", icon: GraduationCap, tint: "#EAB308" },
  { label: "Active Users (24h)", value: "12,981", delta: "-1.2% vs yesterday", trend: "down", icon: Users, tint: "#22C55E" },
  { label: "Platform Uptime", value: "99.98%", delta: "Last 30 days", trend: "up", icon: ActivityIcon, tint: "#22C55E" }
];
const quickActions = [
...
function DashboardPage() {
  ...
  return <div className="mx-auto flex max-w-[1600px] flex-col gap-6">
    ...
    /* Stat cards */
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
        {superAdminStats.map((s) => <StatCard key={s.label} {...s} />)}
      </div>
    ...
    /* Quick actions */
      ...
      <section className="rounded-xl border border-border bg-card p-5 shadow-sm">
        ...
        <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5">
          {quickActions.map((a) => <QuickActionCard key={a.title} title={a.title} description={a.description} icon={a.icon} tint={a.tint} onClick={() => navigate({ to: a.to })} />)}
        </div>
      </section>
  ...
  />
    ...
    paddingAngle={3}
  >
                  {adminDistribution.map((_, i) => <Cell key={i} fill={PIE_COLORS[i % PIE_COLORS.length]} />)}
                </Pie>
                <Tooltip
    contentStyle={{
      background: "var(--card)",
  ...
  />
    ...
    paddingAngle={2}
  >
                  {studentDistribution.map((_, i) => <Cell key={i} fill={PIE_COLORS[i % PIE_COLORS.length]} />)}
                </Pie>
                <Tooltip
    contentStyle={{
      background: "var(--card)",
  ...
  ].map((s) => <li key={s.name} className="flex items-center justify-between py-2.5">
          ...
          <div className="-mx-1 overflow-x-auto">
            <table className="w-full text-left text-sm">
              ...
              <tbody className="divide-y divide-border">
                {recentColleges.map((r) => <tr key={r.name} className="transition-colors hover:bg-muted/40">
