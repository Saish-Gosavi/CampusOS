...
export function WardenAllocationLetterPage() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      const [reqRes, tempRes] = await Promise.all([
        wardenLetterApi.getRequests().catch(() => ({ data: [] })),
  ...
  const handleApprove = async (id) => {
    ...
    try {
      ...
      // Support for frontend testing: mutate dummy data in-memory
      if (String(id).startsWith("dummy-")) {
        const dummyReq = DUMMY_REQUESTS.find(r => r.id === id);
        if (dummyReq) dummyReq.status = "Approved";
      }
  ...
  const handleRejectSubmit = async () => {
    ...
    try {
      ...
      // Support for frontend testing: mutate dummy data in-memory
      if (String(rejectingReq.id).startsWith("dummy-")) {
        const dummyReq = DUMMY_REQUESTS.find(r => r.id === rejectingReq.id);
        if (dummyReq) {
          dummyReq.status = "Rejected";
          dummyReq.rejectionReason = rejectionReason.trim();
        }
      }
  ...
  const handleGeneratePdf = async (reqItem, isRegenerate = false) => {
    ...
    try {
      ...
      // Support for frontend testing: mutate dummy data in-memory
      if (String(reqItem.id).startsWith("dummy-")) {
        const dummyReq = DUMMY_REQUESTS.find(r => r.id === reqItem.id);
        if (dummyReq) {
          dummyReq.status = "Generated";
          dummyReq.allotmentLetter = { pdfPath: letterData.pdfPath, referenceNo: letterData.referenceNo };
        }
      }

      // Open the server-generated PDF directly
      if (letterData?.pdfPath) {
        window.open(BACKEND_URL + letterData.pdfPath, "_blank");
      }
  ...
  const handleOpenPdf = (reqItem) => {
    const pdfPath = reqItem.allotmentLetter?.pdfPath;
    if (!pdfPath) {
      toast.error("PDF file not found. Please regenerate the letter.");
      return;
    }
    
    // Support external URLs (for dummy data) or local paths
