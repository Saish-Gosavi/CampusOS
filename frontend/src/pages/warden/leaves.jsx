...
import { leaveRequests } from "@/lib/hostel-data";
...
function LeavesPage() {
  ...
  const filtered = useMemo(() => {
    const base = tab === "History" ? leaveRequests : leaveRequests.filter((l) => l.status === tab);
    if (!q) return base;
      ...
      <div className="rounded-xl border border-border bg-card p-4 shadow-sm">
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            ...
            <tbody>
              {filtered.map((l) => <tr key={l.id} className="border-b border-border last:border-b-0 hover:bg-muted/40">
  ...
  }} title="View details" className="grid h-7 w-7 place-items-center rounded-md text-muted-foreground hover:bg-muted hover:text-foreground"><Eye className="h-3.5 w-3.5" /></button>
                      {l.status === "Pending" && <>
                          <button onClick={() => {
    setSelected(l);
    setRemarks("");
  }} title="Approve" className="grid h-7 w-7 place-items-center rounded-md bg-[#22C55E]/10 text-[#16A34A] hover:bg-[#22C55E]/20"><CheckCircle2 className="h-3.5 w-3.5" /></button>
                          <button onClick={() => {
    setSelected(l);
    setRemarks("");
  }} title="Reject" className="grid h-7 w-7 place-items-center rounded-md bg-[#EF4444]/10 text-[#DC2626] hover:bg-[#EF4444]/20"><XCircle className="h-3.5 w-3.5" /></button>
                        </>}
                    </div>
                  </td>
                </tr>)}
              {filtered.length === 0 && <tr><td colSpan={9} className="py-10 text-center text-muted-foreground">No leave requests</td></tr>}
            </tbody>
          </table>
        </div>
      </div>
