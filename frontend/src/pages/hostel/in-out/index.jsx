...
import { InOutTable } from "@/components/hostel/in-out/InOutTable";
import { NewLogDialog } from "@/components/hostel/in-out/NewLogDialog";
import axios from "axios";

const Route = createFileRoute("/hostel-admin/in-out")({
  component: InOutRegisterPage
});

function InOutRegisterPage() {
  ...
  const fetchLogs = async () => {
    try {
      const res = await axios.get("http://localhost:5000/api/inout", { withCredentials: true });
      if (res.data.success) {
  ...
  const handleAddLog = async (newLog) => {
    try {
      const res = await axios.post("http://localhost:5000/api/inout", newLog, { withCredentials: true });
      if (res.data.success) {
        fetchLogs();
      }
    } catch (err) {
      ...
      <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-muted-foreground">
            Monitor real-time entry and exit logs for all students, staff, and visitors.
          </p>
        </div>
...
export default InOutRegisterPage;
export { Route };
