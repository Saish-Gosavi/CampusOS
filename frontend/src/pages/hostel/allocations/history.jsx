...
const Route = createFileRoute("/hostel-admin/allocation/history")({
...
function AllocationHistoryPage() {
  ...
  const rows = useMemo(() => {
    return [...allocations].sort((a, b) => a.allocatedOn < b.allocatedOn ? 1 : -1).filter((a) => status === "all" ? true : a.status === status).filter((a) => {
  ...
  >
    ...
    return <li key={a.id} className="flex flex-col gap-3 p-4 sm:flex-row sm:items-center sm:justify-between">
                <div className="flex min-w-0 items-center gap-3">
                  ...
                  <div className="min-w-0">
                    ...
                    {a.note && <p className="mt-0.5 truncate text-[11px] italic text-muted-foreground">"{a.note}"</p>}
                  </div>
                </div>
                <div className="flex flex-wrap items-center gap-3 sm:justify-end">
                  ...
                  {a.vacatedOn && <div className="text-right text-xs">
          ...
          {rows.length === 0 && <li className="p-10 text-center text-sm text-muted-foreground">No records found.</li>}
        </ol>
      </div>
    </div>;
}
