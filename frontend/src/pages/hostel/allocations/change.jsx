...
const Route = createFileRoute("/hostel-admin/allocation/change")({
...
function RoomChangePage() {
  ...
  >
    ...
    return <option key={a.id} value={a.id}>
        ...
        {alloc && student && <div className="mt-4 rounded-lg border border-border bg-muted/30 p-4">
            ...
            <dl className="mt-3 grid grid-cols-2 gap-2 text-xs">
              ...
              <Info label="Bed" value={curBed ? `${curBed.number} \xB7 ${curBed.type}` : "\u2014"} />
  ...
  />
    ...
    options={roomOpts.map((r) => ({
      value: r.id,
      label: `${r.number} \xB7 ${r.beds - r.occupied} free`
    }))}
  ...
  />
    ...
    return <button
      ...
      className={cn(
        ...
        selected ? "border-[#7B4CED] bg-[#7B4CED]/5 ring-2 ring-[#7B4CED]/30" : "border-border hover:border-[#7B4CED]/40"
      )}
    >
              ...
              {newRoomId && bedOpts.length === 0 && <p className="col-span-full py-3 text-center text-xs text-muted-foreground">
                  No beds available in this room.
                </p>}
              {!newRoomId && <p className="col-span-full py-3 text-center text-xs text-muted-foreground">
  ...
  />
        ...
        {newBed && <div className="mt-4 flex items-center justify-between gap-3 rounded-lg border border-dashed border-[#7B4CED]/50 bg-[#7B4CED]/5 p-3 text-sm">
            <div className="min-w-0">
