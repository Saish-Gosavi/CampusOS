...
const notifications = [
  { icon: BookMarked, tint: "#7B4CED", title: "Reservation ready for pickup", meta: "Deep Work \u2014 Kabir Verma" },
  { icon: IndianRupee, tint: "#EAB308", title: "Fine payment received", meta: "\u20B980 \u2014 Riya Shah" },
  { icon: Inbox, tint: "#0D9488", title: "New book request", meta: "System Design Interview Vol. 2" }
];
...
function LibraryNavbar() {
  ...
  return <header className="sticky top-0 z-20 flex h-16 items-center gap-2 border-b border-border bg-card/90 px-3 backdrop-blur sm:px-6">
    ...
    className="h-10 rounded-lg border-border bg-muted/40 pl-9 focus-visible:ring-[#0D9488]"
  />
      </div>
  ...
  >
          ...
          <DropdownMenuContent align="end" className="w-80">
            <DropdownMenuLabel className="flex items-center justify-between">
              ...
              <span className="rounded-full bg-[#0D9488]/10 px-2 py-0.5 text-xs font-medium text-[#0D9488]">
                3 new
              </span>
            </DropdownMenuLabel>
            <DropdownMenuSeparator />
  ...
  >
            ...
            <DropdownMenuSeparator />
            <DropdownMenuItem className="justify-center text-sm font-medium text-[#0D9488]">
        ...
        <DropdownMenu>
          <DropdownMenuTrigger asChild>
            <button className="ml-1 flex items-center gap-2 rounded-lg py-1 pl-1 pr-2 transition-colors hover:bg-muted">
              <span className="grid h-8 w-8 place-items-center rounded-full bg-gradient-to-br from-[#0D9488] to-[#7B4CED] text-sm font-semibold text-white">
                LA
              </span>
              <span className="hidden text-left leading-tight sm:block">
                <span className="block text-sm font-semibold text-foreground">Library Admin</span>
                <span className="block text-xs text-muted-foreground">library@vppcoe.edu</span>
              </span>
          ...
          <DropdownMenuContent align="end" className="w-56">
            ...
            <DropdownMenuItem asChild>
              <Link to="/library-admin/profile">
                <UserCircle2 className="mr-2 h-4 w-4" /> Profile
              </Link>
            </DropdownMenuItem>
            <DropdownMenuItem asChild>
              <Link to="/library-admin/settings">
