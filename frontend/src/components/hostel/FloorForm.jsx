...
import { blocks, hostels, floors } from "@/lib/hostel-data";
function FloorForm({ mode, floor }) {
  ...
  const currentBlockId = availableBlocks.some((b) => b.id === form.blockId) ? form.blockId : availableBlocks[0]?.id ?? "";
  const set = (k, v) => setForm((f) => ({ ...f, [k]: v }));
  const onSubmit = (e) => {
    e.preventDefault();
    navigate("/hostel-admin/floors");
  };
