...
const TYPES = ["Single", "Bunk-Upper", "Bunk-Lower"];
function BedForm({ mode, bed }) {
  ...
  const [form, setForm] = useState({
    ...
    type: mode === "edit" ? initial.type : "Single",
  ...
  />
          ...
          <Field
    label="Room"
    hint={roomMeta ? `${roomMeta.hostel?.name ?? "\u2014"} \xB7 ${roomMeta.block?.name ?? "\u2014"} \xB7 Floor ${roomMeta.room.floor ?? "\u2014"}` : void 0}
  >
            <select
    required
    className={selectCls}
  ...
  >
    ...
    hint={!showStudent ? "Available or Maintenance beds are not assigned to students." : void 0}
  >
            <select
    className={selectCls}
