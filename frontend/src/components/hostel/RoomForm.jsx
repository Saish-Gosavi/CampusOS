...
import { hostels, blocks, floors, rooms } from "@/lib/hostel-data";
const ROOM_TYPES = ["Single", "Double", "Triple", "Dormitory", "Deluxe"];
const AMENITY_OPTIONS = ["Wi-Fi", "Attached Bath", "AC", "Study Table", "Wardrobe", "Balcony", "Mini Fridge", "Lockers"];
function RoomForm({ mode, room }) {
  ...
  const currentBlockId = availableBlocks.some((b) => b.id === blockId) ? blockId : availableBlocks[0]?.id ?? "";
  const availableFloors = useMemo(
    () => floors.filter((f) => f.blockId === currentBlockId),
    [currentBlockId]
  );
  const currentFloorId = availableFloors.some((f) => f.id === form.floorId) ? form.floorId : availableFloors[0]?.id ?? "";
  ...
  />
    ...
    return <button
      ...
      className={`rounded-full border px-3 py-1 text-xs font-medium transition-colors ${active ? "border-transparent bg-[#7B4CED] text-white" : "border-border bg-background text-foreground hover:bg-muted"}`}
    >
                {a}
              </button>;
  })}
