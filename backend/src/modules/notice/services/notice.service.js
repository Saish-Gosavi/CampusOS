...
export class NoticeService {
  ...
  static async update(id, data, userId) {
    ...
    const notice = await NoticeRepository.update(id, updateData);
    await audit(userId, "UPDATE_NOTICE", { id, ...data });
    return notice;
  }

  static async remove(id, userId) {
    await NoticeRepository.delete(id);
    await audit(userId, "DELETE_NOTICE", { id });
  }
}
