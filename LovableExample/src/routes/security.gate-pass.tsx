...
import { gatePasses, type GatePass } from "@/lib/security-data";

export const Route = createFileRoute("/security/gate-pass")({
...
function GatePassPage() {
  ...
  const filter = (l: GatePass[]) =>
    l.filter((g) => !q || g.id.toLowerCase().includes(q.toLowerCase()) || g.student.toLowerCase().includes(q.toLowerCase()));
...
function VerifyCard() {
  ...
  return (
    <div className="rounded-xl border border-border bg-card p-5 shadow-sm">
      ...
      {notFound && (
        <div className="mt-4 rounded-lg border border-[#EF4444]/30 bg-[#EF4444]/10 p-3 text-sm text-[#DC2626]">
      ...
      {result && (
        <div className="mt-4 rounded-xl border border-border bg-muted/20 p-4">
          ...
          <dl className="mt-3 space-y-1.5 text-xs">
            <Row k="Student" v={`${result.student} · ${result.enrollment}`} />
...
function PassTable({ rows, showActions }: { rows: GatePass[]; showActions?: boolean }) {
  return (
