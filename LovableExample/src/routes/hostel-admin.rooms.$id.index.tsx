...
export const Route = createFileRoute("/hostel-admin/rooms/$id/")({
  ...
  return (
    <div className="mx-auto flex max-w-[1400px] flex-col gap-6">
      <HostelPageHeader
        ...
        description={`${hostel?.name ?? "Hostel"} · ${block?.name ?? ""} · Floor ${floor?.number ?? room.floor ?? "—"}`}
        icon={DoorClosed}
        tint={typeTint}
        breadcrumbs={[
            ...
            {room.amenities && room.amenities.length > 0 ? (
              <div className="flex flex-wrap gap-2">
                {room.amenities.map((a) => (
                  <span key={a} className="inline-flex items-center gap-1.5 rounded-full border border-border bg-background px-3 py-1 text-xs font-medium text-foreground">
        ...
        <div className="flex flex-col gap-6">
          ...
          <div className="rounded-xl border border-border bg-card p-6 shadow-sm">
            ...
            <div className="flex flex-col gap-3">
              ...
              {room.rent !== undefined && (
                <InfoRow icon={IndianRupee} label="Monthly Rent" value={`₹${room.rent.toLocaleString("en-IN")}`} />
              )}
            </div>
          </div>
        </div>
