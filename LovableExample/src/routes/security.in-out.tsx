...
import { inOutEntries, type InOutEntry } from "@/lib/hostel-data";

export const Route = createFileRoute("/security/in-out")({
  component: InOutPage,
});
...
function InOutPage() {
  ...
  const filter = (list: InOutEntry[]) =>
    list.filter((e) =>
      !q ||
      e.student.toLowerCase().includes(q.toLowerCase()) ||
      e.enrollment.toLowerCase().includes(q.toLowerCase()) ||
      e.room.toLowerCase().includes(q.toLowerCase()),
    );
...
function MovementTable({ rows, mode }: { rows: InOutEntry[]; mode: "in" | "out" | "history" }) {
  return (
      ...
      <div className="overflow-x-auto">
        <table className="w-full text-sm">
          ...
          <tbody className="divide-y divide-border">
            ...
            {rows.length === 0 && (
              <tr><td colSpan={9} className="px-4 py-10 text-center text-sm text-muted-foreground">No records found · mode: {mode}</td></tr>
            )}
          </tbody>
        </table>
      </div>
