...
export const Route = createFileRoute("/hostel-admin/allocation/new")({
...
function AllocateWizardPage() {
  ...
  const submit = () => {
    ...
    toast.success(
      `${student.name} allocated → ${hostel.name} · ${block.name} · Floor ${floor.number} · Room ${room.number} · ${bed.number}`,
    );
    navigate({ to: "/hostel-admin/allocation" });
  };

  const availPct = (occ: number, cap: number) =>
    cap === 0 ? 0 : Math.round(((cap - occ) / cap) * 100);
    ...
    <div className="grid grid-cols-1 gap-6 lg:grid-cols-[1fr_320px]">
      ...
      <div className="flex flex-col gap-5">
                    ...
                    <button
                      ...
                      className={cn(
                        ...
                        selected
                          ...
                          : "border-border hover:border-[#7B4CED]/40 hover:bg-muted/40",
                      )}
                    >
        ...
        <div className="flex items-center justify-between">
          ...
          {step < 5 ? (
            <Button className="bg-[#7B4CED] hover:bg-[#6a3dd6]" onClick={next} disabled={!canNext}>
      ...
      <aside className="rounded-xl border border-border bg-card p-5 shadow-sm lg:sticky lg:top-24 lg:self-start">
        ...
        <ul className="mt-4 space-y-3 text-sm">
          <SummaryRow icon={User} label="Student" value={student ? `${student.name} · ${student.enrollment}` : "—"} />
          ...
          />
          <SummaryRow icon={BedDouble} label="Bed" value={bed ? `${bed.number} · ${bed.type}` : "—"} />
        </ul>
        {room && (
