...
export const Route = createFileRoute("/hostel-admin/rooms/")({
...
function RoomsListPage() {
  ...
  const blockName = (id?: string) => blocks.find((b) => b.id === id)?.name ?? `Block ${allRooms.find((r) => r.blockId === id)?.block ?? ""}`;
  const floorNum = (id?: string) => floors.find((f) => f.id === id)?.number;
      ...
      <div className="rounded-xl border border-border bg-card p-4 shadow-sm">
        <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            <select
              value={hostelId}
              onChange={(e) => { setHostelId(e.target.value); setBlockId("All"); }}
      ...
      {view === "grid" ? (
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
          {filtered.map((r) => {
            ...
            return (
              <div key={r.id} className="group flex flex-col overflow-hidden rounded-xl border border-border bg-card shadow-sm transition-shadow hover:shadow-md">
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              ...
              <tbody>
                ...
                {filtered.length === 0 && (
                  <tr><td colSpan={10} className="px-4 py-10 text-center text-sm text-muted-foreground">No rooms match your filters.</td></tr>
                )}
              </tbody>
            </table>
          </div>
