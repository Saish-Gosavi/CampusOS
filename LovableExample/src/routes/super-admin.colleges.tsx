...
export const Route = createFileRoute("/super-admin/colleges")({
...
const seed: College[] = recentColleges.map((c, i) => ({
  id: `c${i + 1}`,
  name: c.name,
  city: c.city,
  students: c.students,
...
function CollegesPage() {
  const [colleges, setColleges] = useState<College[]>(seed);
  ...
  function save(data: Omit<College, "id">) {
    if (editing) {
...
function CollegeModal({
  ...
  onClose: () => void;
  onSave: (data: Omit<College, "id">) => void;
}) {
