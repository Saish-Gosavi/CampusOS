    ...
    return students.filter((s) => {
      ...
      if (q) {
        ...
        return (
          ...
          s.enrollment.toLowerCase().includes(t) ||
      ...
      <div className="rounded-xl border border-border bg-card p-4 shadow-sm">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center">
          <div className="relative flex-1">
            ...
            <Input value={q} onChange={(e) => { setQ(e.target.value); setPage(1); }} placeholder="Search by name, enrollment or room..." className="h-10 pl-9" />
          </div>
          <select value={dept} onChange={(e) => { setDept(e.target.value); setPage(1); }} className="h-10 rounded-lg border border-border bg-background px-3 text-sm">
        ...
        <div className="mt-4 overflow-x-auto">
          <table className="w-full text-sm">
            ...
            <tbody>
              ...
              {paged.length === 0 && (
                <tr><td colSpan={9} className="py-10 text-center text-muted-foreground">No students found</td></tr>
              )}
            </tbody>
          </table>
        </div>
