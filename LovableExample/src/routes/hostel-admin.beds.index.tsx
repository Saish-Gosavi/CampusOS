...
export const Route = createFileRoute("/hostel-admin/beds/")({
  component: BedsPage,
});
...
function BedsPage() {
  ...
  const filtered = useMemo(() => {
    ...
    return allBeds.filter((b) => {
      ...
      const stu = b.studentId ? studentMap[b.studentId] : undefined;
      const mQ =
        !query ||
        b.number.toLowerCase().includes(query) ||
        room?.number.toLowerCase().includes(query) ||
        stu?.name.toLowerCase().includes(query) ||
        stu?.enrollment.toLowerCase().includes(query);
  ...
  const stats = useMemo(() => {
    ...
    const count = (s: string) => allBeds.filter((b) => b.status === s).length;
    return {
      total,
          ...
          <table className="w-full text-sm">
            ...
            <tbody>
              {filtered.map((b) => {
                const room = roomMap[b.roomId];
                const stu = b.studentId ? studentMap[b.studentId] : undefined;
                const hostel = room ? hostelMap[room.hostelId ?? ""] : undefined;
                const block = room ? blockMap[room.blockId ?? ""] : undefined;
                return (
                  <tr
                    key={b.id}
