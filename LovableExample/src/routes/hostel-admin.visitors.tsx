...
export const Route = createFileRoute("/hostel-admin/visitors")({
  head: () => ({
    meta: [
      { title: "Visitor Management — Hostel Admin · CampusOS" },
      {
        name: "description",
        content:
          "Manage hostel visitor requests, approvals, on-campus check-in / check-out and full visitor history.",
      },
    ],
  }),
...
type Tab = "Requests" | "Approval" | "History";

const TINT = "#0EA5E9";

function VisitorsPage() {
  const [rows, setRows] = useState<VisitorRequest[]>(seed);
  const [tab, setTab] = useState<Tab>("Requests");
  const [q, setQ] = useState("");
  const [relation, setRelation] = useState<string>("All");

  const [viewing, setViewing] = useState<VisitorRequest | null>(null);
  ...
  const filtered = useMemo(() => {
    return rows.filter((r) => {
      ...
      if (
        tab === "Approval" &&
        !(r.status === "Approved" || r.status === "Checked-In")
      )
        return false;
      if (
        tab === "History" &&
        !(r.status === "Checked-Out" || r.status === "Rejected" || r.status === "Expired")
      )
        return false;
  ...
  const openReview = (row: VisitorRequest, action: "Approved" | "Rejected") => {
      ...
      prev.map((r) =>
        r.id === row.id
          ? {
              ...
              remarks:
                remarks.trim() ||
                (action === "Approved" ? "Approved — visitor allowed on campus." : "Rejected."),
  ...
  const setStatus = (row: VisitorRequest, status: VisitorStatus, msg: string) => {
    setRows((prev) =>
    ...
    <div className="mx-auto flex max-w-[1600px] flex-col gap-6">
      ...
      <div className="rounded-xl border border-border bg-card shadow-sm">
        <div className="flex flex-wrap items-center gap-2 border-b border-border p-3">
          {(["Requests", "Approval", "History"] as Tab[]).map((t) => (
            <button
              key={t}
              onClick={() => setTab(t)}
          ...
          {viewing && (
            <>
              ...
              <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                ...
                <InfoRow icon={User} label="Student" value={`${viewing.student} · ${viewing.enrollment}`} />
                <InfoRow icon={User} label="Room" value={`${viewing.hostel} · ${viewing.room}`} />
                ...
                <Textarea
                  ...
                  placeholder={
                    reviewing.action === "Approved"
                      ? "Optional notes for gate security…"
