...
export const Route = createFileRoute("/hostel-admin/reports")({
...
function ReportsPage() {
  ...
  const download = (fmt: string) => toast.success(`Report queued for download (${fmt})`);
    ...
    <div className="mx-auto flex max-w-[1600px] flex-col gap-6">
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <KpiCard
          ...
          value={`₹${(totals.collected / 100000).toFixed(1)}L`}
          delta={`₹${(totals.pendingAmt / 100000).toFixed(1)}L pending`}
          trend="up"
          icon={IndianRupee}
          tint="#22C55E"
        />
            ...
            <ResponsiveContainer width="100%" height="100%">
              <PieChart>
                <Pie data={complaintOverview} dataKey="value" nameKey="name" innerRadius={50} outerRadius={95} paddingAngle={2}>
                  {complaintOverview.map((_, i) => (
                    <Cell key={i} fill={["#EF4444", "#EAB308", "#22C55E"][i % 3]} />
                  ))}
                </Pie>
                <Tooltip contentStyle={tooltipStyle} />
                <Legend wrapperStyle={{ fontSize: 12 }} />
        ...
        <ChartCard title="Leave Report" description="Applications by type and status">
          <div className="grid h-72 grid-cols-2 gap-3">
            ...
            <div className="flex flex-col justify-center gap-2 text-xs">
              {leaveRequestOverview.map((r, i) => (
                <div key={r.name} className="flex items-center justify-between rounded-lg border border-border bg-muted/30 px-3 py-2">
                  <div className="flex items-center gap-2">
