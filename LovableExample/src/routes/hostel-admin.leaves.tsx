...
export const Route = createFileRoute("/hostel-admin/leaves")({
  head: () => ({
    meta: [
...
type Tab = "All" | "Pending" | "Approved" | "Rejected" | "History";
...
function LeavesPage() {
  ...
  const [reviewing, setReviewing] = useState<{ row: LeaveRequest; action: "Approved" | "Rejected" } | null>(null);
  ...
  const openReview = (row: LeaveRequest, action: "Approved" | "Rejected") => {
    ...
    <div className="mx-auto flex max-w-[1600px] flex-col gap-6">
      ...
      <div className="rounded-xl border border-border bg-card shadow-sm">
        <div className="flex flex-wrap items-center gap-2 border-b border-border p-3">
          {(["Pending", "Approved", "Rejected", "All", "History"] as Tab[]).map((t) => (
            <button
              key={t}
              onClick={() => setTab(t)}
                ...
                <Textarea
                  ...
                  placeholder={
                    reviewing.action === "Approved"
                      ? "Optional notes for the resident…"
