...
import { students, type Student } from "@/lib/hostel-data";

export const Route = createFileRoute("/hostel-admin/students/")({
...
function exportCsv(rows: Student[]) {
  ...
  const body = rows.map((s) =>
    [s.name, s.enrollment, s.department, s.year, s.gender, s.hostel, s.room, s.contact, s.parentContact, s.status]
      ...
      <div className="rounded-xl border border-border bg-card p-4 shadow-sm">
        <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            <select value={dept} onChange={(e) => { setDept(e.target.value); setPage(1); }} className="h-10 rounded-lg border border-border bg-background px-3 text-sm text-foreground">
              {depts.map((d) => <option key={d}>{d}</option>)}
            </select>
            <select value={year} onChange={(e) => { setYear(e.target.value); setPage(1); }} className="h-10 rounded-lg border border-border bg-background px-3 text-sm text-foreground">
              {years.map((y) => <option key={y} value={y}>{y === "All" ? "All Years" : `Year ${y}`}</option>)}
            </select>
            <select value={status} onChange={(e) => { setStatus(e.target.value); setPage(1); }} className="h-10 rounded-lg border border-border bg-background px-3 text-sm text-foreground">
