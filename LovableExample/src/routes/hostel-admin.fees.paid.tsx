...
export const Route = createFileRoute("/hostel-admin/fees/paid")({
...
const METHODS = ["All Methods", "UPI", "Card", "Net Banking", "Cash", "DD"] as const;

function PaidFeesPage() {
  const [method, setMethod] = useState<(typeof METHODS)[number]>("All Methods");
    ...
    if (query.trim()) {
      ...
      list = list.filter(
        (f) =>
          ...
          (f.receipt ?? "").toLowerCase().includes(q),
      );
    }
    return list;
  }, [method, query]);
