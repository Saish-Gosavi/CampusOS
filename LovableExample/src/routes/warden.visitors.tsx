...
export const Route = createFileRoute("/warden/visitors")({
  component: VisitorsPage,
});

const TINT = "#06B6D4";

const tabs = ["Today", "Requests", "History"] as const;
      ...
      <div className="rounded-xl border border-border bg-card p-4 shadow-sm">
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            ...
            <tbody>
              {filtered.map((v) => (
                <tr key={v.id} className="border-b border-border last:border-b-0 hover:bg-muted/40">
                  ...
                  <td className="py-3 pr-2">
                    <div className="flex items-center justify-end gap-1">
                      ...
                      {v.status === "Pending" && (
                        <>
                          <button onClick={() => { setSelected(v); }} className="grid h-7 w-7 place-items-center rounded-md bg-[#22C55E]/10 text-[#16A34A] hover:bg-[#22C55E]/20"><CheckCircle2 className="h-3.5 w-3.5" /></button>
                          <button onClick={() => { setSelected(v); }} className="grid h-7 w-7 place-items-center rounded-md bg-[#EF4444]/10 text-[#DC2626] hover:bg-[#EF4444]/20"><XCircle className="h-3.5 w-3.5" /></button>
                        </>
                      )}
                    </div>
                  </td>
              ...
              {filtered.length === 0 && (
                <tr><td colSpan={9} className="py-10 text-center text-muted-foreground">No visitor records</td></tr>
              )}
            </tbody>
          </table>
        </div>
