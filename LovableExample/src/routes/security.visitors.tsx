...
import { visitorRequests, type VisitorRequest } from "@/lib/hostel-data";

export const Route = createFileRoute("/security/visitors")({
...
function VisitorsPage() {
  ...
  const [active, setActive] = useState<VisitorRequest | null>(null);

  const filter = (l: VisitorRequest[]) =>
    l.filter((v) => !q || v.visitorName.toLowerCase().includes(q.toLowerCase()) || v.student.toLowerCase().includes(q.toLowerCase()));
          ...
          {active && (
            <>
              ...
              <div className="space-y-3 text-sm">
                ...
                <Row label="Meeting" value={`${active.relation} · ${active.student}`} />
                <Row label="Room" value={`${active.hostel} · ${active.room}`} />
                <Row label="Purpose" value={active.purpose} />
                <Row label="Slot" value={`${active.visitDate} · ${active.entryTime} → ${active.exitTime}`} />
...
function VisitorTable({ rows, action, onView }: { rows: VisitorRequest[]; action?: "check-in" | "check-out"; onView: (v: VisitorRequest) => void }) {
  return (
      ...
      <div className="overflow-x-auto">
        <table className="w-full text-sm">
          ...
          <tbody className="divide-y divide-border">
              ...
              <tr key={v.id} className="hover:bg-muted/30">
                ...
                <td className="px-4 py-3 text-right">
                  <div className="flex items-center justify-end gap-1">
                    {action === "check-in" && (
                      <Button size="sm" className="h-7 gap-1 bg-[#22C55E] text-white hover:bg-[#16A34A]">
                        <LogIn className="h-3.5 w-3.5" /> Check In
                      </Button>
                    )}
                    {action === "check-out" && (
                      <Button size="sm" className="h-7 gap-1 bg-[#F97316] text-white hover:bg-[#EA580C]">
            ...
            {rows.length === 0 && (
              <tr><td colSpan={8} className="px-4 py-10 text-center text-sm text-muted-foreground">No visitors here.</td></tr>
            )}
          </tbody>
        </table>
      </div>
