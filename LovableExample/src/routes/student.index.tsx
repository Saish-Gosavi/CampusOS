...
const stats = [
  { label: "Room Number", value: studentProfile.hostel.room, delta: `${studentProfile.hostel.name}`, trend: "up" as const, icon: BedDouble, tint: "#2563EB" },
  { label: "Leave Requests", value: "1 pending", delta: "2 approved this term", trend: "up" as const, icon: CalendarDays, tint: "#F97316" },
  { label: "Pending Complaints", value: "2", delta: "1 in progress", trend: "down" as const, icon: MessageSquareWarning, tint: "#EF4444" },
  { label: "Hostel Fees", value: "₹42,000", delta: "Due 15 Aug", trend: "down" as const, icon: IndianRupee, tint: "#22C55E" },
  { label: "Borrowed Books", value: "3", delta: "1 overdue", trend: "down" as const, icon: BookOpen, tint: "#0D9488" },
  { label: "Books Due Soon", value: "2", delta: "Within a week", trend: "up" as const, icon: Clock, tint: "#7B4CED" },
  { label: "Library Fine", value: "₹40", delta: "Pending", trend: "down" as const, icon: AlertTriangle, tint: "#EAB308" },
  { label: "Notifications", value: "6", delta: "3 unread", trend: "up" as const, icon: Bell, tint: "#0EA5E9" },
];
  ...
  return (
    <div className="mx-auto flex max-w-[1600px] flex-col gap-6">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <StudentBreadcrumbs items={[{ label: "Dashboard" }]} />
        ...
        <ChartCard title="Upcoming due dates" description="Stay on top of deadlines">
          <ul className="space-y-3">
            {upcomingDueDates.map((d) => (
              <li key={d.id} className="flex items-center gap-3 rounded-lg border border-border bg-background/60 p-3">
      ...
      <div className="grid grid-cols-1 gap-4 lg:grid-cols-3">
        <ChartCard
          ...
          action={
            <Link to="/student/borrowed" className="text-xs font-medium text-[#2563EB] hover:underline">
              View all
            </Link>
          }
        >
        ...
        <ChartCard
          ...
          action={
            <Link to="/student/complaints" className="text-xs font-medium text-[#2563EB] hover:underline">
              View all
            </Link>
          }
        >
        ...
        <ChartCard
          ...
          action={
            <Link to="/student/leaves" className="text-xs font-medium text-[#2563EB] hover:underline">
              View all
            </Link>
          }
        >
      ...
      <div className="grid grid-cols-1 gap-4 lg:grid-cols-3">
        <ChartCard
          ...
          action={
            <Link to="/student/hostel-notices" className="inline-flex items-center gap-1 text-xs font-medium text-[#2563EB] hover:underline">
              All <ArrowRight className="h-3 w-3" />
            </Link>
          }
        >
          <ul className="space-y-3">
            {hostelNoticesFeed.slice(0, 3).map((n) => (
        ...
        <ChartCard
          ...
          className="lg:col-span-1"
          action={
            <Link to="/student/library-notices" className="inline-flex items-center gap-1 text-xs font-medium text-[#2563EB] hover:underline">
              All <ArrowRight className="h-3 w-3" />
            </Link>
          }
        >
          <ul className="space-y-3">
            {libraryNoticesFeed.map((n) => (
              <li key={n.id} className="rounded-lg border border-border p-3">
                <div className="flex items-center gap-2">
        ...
        <ChartCard title="Recent activity" description="Your latest interactions" className="lg:col-span-1">
          <ActivityTimeline items={studentActivities.slice(0, 6) as never} />
        </ChartCard>
      </div>
    </div>
  );
