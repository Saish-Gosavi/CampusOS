...
import type { Notice, NoticeAudience, NoticeCategory, NoticeStatus } from "@/lib/hostel-data";

const TINT = "#EAB308";

const audiences: NoticeAudience[] = ["All Blocks", "Block A", "Block B", "Block C", "Block D", "Wardens", "Staff"];
const categories: NoticeCategory[] = ["General", "Maintenance", "Event", "Emergency", "Academic"];
...
interface Props {
  ...
  onSubmit: (values: NoticeFormValues, status: NoticeStatus) => void;
  onPreview?: (values: NoticeFormValues) => void;
  submittingLabel?: string;
}

export function NoticeForm({ initial, onSubmit, onPreview }: Props) {
  const [title, setTitle] = useState(initial?.title ?? "");
  const [body, setBody] = useState(initial?.body ?? "");
  const [audience, setAudience] = useState<NoticeAudience>((initial?.audience as NoticeAudience) ?? "All Blocks");
  const [category, setCategory] = useState<NoticeCategory>((initial?.category as NoticeCategory) ?? "General");
  const [publishDate, setPublishDate] = useState(initial?.publishDate ?? new Date().toISOString().slice(0, 10));
  const [expiryDate, setExpiryDate] = useState(initial?.expiryDate ?? "");
  const [attachment, setAttachment] = useState<{ name: string; size: string } | null>(initial?.attachment ?? null);
  const [pinned, setPinned] = useState(initial?.pinned ?? false);

  const values: NoticeFormValues = { title, body, audience, category, publishDate, expiryDate, attachment, pinned };

  const handle = (status: NoticeStatus) => (e: FormEvent) => {
    e.preventDefault();
    onSubmit(values, status);
  };

  return (
    <form className="grid grid-cols-1 gap-6 lg:grid-cols-[1.4fr_1fr]" onSubmit={handle("Published")}>
      ...
      <div className="flex flex-col gap-4">
        ...
        <div className="rounded-xl border border-border bg-card p-5 shadow-sm">
          ...
          <div className="mt-3 flex flex-col gap-2">
            ...
            <Button type="button" variant="outline" onClick={handle("Draft") as any}>
              <Save className="mr-1.5 h-4 w-4" /> Save as Draft
            </Button>
            {onPreview && (
              <Button type="button" variant="ghost" onClick={() => onPreview(values)}>
