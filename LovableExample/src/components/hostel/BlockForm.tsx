...
import { hostels, blocks, type Block } from "@/lib/hostel-data";

type Props = { mode: "add" | "edit"; block?: Block };

export function BlockForm({ mode, block }: Props) {
    ...
    <form onSubmit={onSubmit} className="flex flex-col gap-6">
      <div className="rounded-xl border border-border bg-card p-6 shadow-sm">
        ...
        <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
          ...
          <Field label="Status">
            <select className={selectCls} value={form.status} onChange={(e) => set("status", e.target.value as Block["status"])}>
