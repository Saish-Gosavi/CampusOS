...
import type { Student } from "@/lib/hostel-data";

type Draft = Partial<Student> & {
  fatherName?: string; fatherOccupation?: string; fatherContact?: string;
  motherName?: string; motherOccupation?: string; motherContact?: string;
  emergencyName?: string; emergencyRelation?: string; emergencyContact?: string;
};
    ...
    <form onSubmit={handleSubmit} className="flex flex-col gap-5">
      <Section title="Personal Information" icon={<User className="h-4 w-4" />}>
        ...
        <Field label="Gender" required>
          <select className={selectCls} value={d.gender} onChange={(e) => set("gender", e.target.value as Student["gender"])}>
      ...
      <Section title="Hostel Details" icon={<Building2 className="h-4 w-4" />}>
        <Field label="Hostel" required>
          <select className={selectCls} value={d.hostel} onChange={(e) => set("hostel", e.target.value as Student["hostel"])}>
        ...
        <Field label="Status">
          <select className={selectCls} value={d.status} onChange={(e) => set("status", e.target.value as Student["status"])}>
      ...
      <Section title="Documents Upload" icon={<FileText className="h-4 w-4" />}>
        {["Photograph", "ID Proof (Aadhaar)", "Admission Letter", "Fee Receipt"].map((doc) => (
          <div key={doc} className="flex items-center justify-between rounded-lg border border-dashed border-border bg-muted/30 px-4 py-3">
            <div>
