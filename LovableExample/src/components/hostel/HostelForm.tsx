...
function Field({ label, children, required, span }: { label: string; children: ReactNode; required?: boolean; span?: boolean }) {
  return (
    <label className={`flex flex-col gap-1.5 text-sm ${span ? "md:col-span-2" : ""}`}>
...
export function HostelForm({ initial, mode }: Props) {
  ...
  const [d, setD] = useState({
    ...
    establishedYear: initial?.establishedYear ?? new Date().getFullYear(),
    ...
    <form onSubmit={submit} className="flex flex-col gap-5">
      <Section title="Hostel Information" icon={<Building2 className="h-4 w-4" />}>
        ...
        <Field label="Hostel Type" required>
          <select className={selectCls} value={d.type} onChange={(e) => set("type", e.target.value as typeof d.type)}>
        ...
        <Field label="Status" required>
          <select className={selectCls} value={d.status} onChange={(e) => set("status", e.target.value as typeof d.status)}>
